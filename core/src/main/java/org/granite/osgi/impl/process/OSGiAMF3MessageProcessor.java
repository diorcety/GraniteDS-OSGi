...
package org.granite.osgi.impl.process;
 ...
 * @author Franck WOLFF
 */
public abstract class OSGiAMF3MessageProcessor {
    ...
    public static Message process(Message request) {
        ...
        try {
            ...
            if (request instanceof RemotingMessage)
                ...
                throw new IllegalArgumentException(
                        "Unknown request message type: " + request);
        } finally {
            if (interceptor != null)
        ...
        if (context instanceof HttpGraniteContext) {
            HttpSession session = ((HttpGraniteContext) context).getRequest().getSession(
                    false);
            if (session != null)
    ...
    public static Message processCommandMessage(CommandMessage request) {
        ...
        if (request.isSecurityOperation()) {
            ...
            if (!config.hasSecurityService())
                log.warn(
                        "Ignored security operation (no security settings in granite-config.xml): %s",
                        request);
            else {
        ...
        if ("nil".equals(request.getHeader(Message.DS_ID_HEADER)))
            response.getHeaders().put(
                    Message.DS_ID_HEADER, UUIDUtil.randomUUID());
    ...
    public static Message processRemotingMessage(RemotingMessage request) {
        ...
        try {
            ...
            GraniteContext context = GraniteContext.getCurrentInstance();
            IMainFactory mainFactory = (IMainFactory) context.getRequestMap().get("mainFactory");
            ServiceFactory factory = mainFactory.getFactoryInstance(request);
